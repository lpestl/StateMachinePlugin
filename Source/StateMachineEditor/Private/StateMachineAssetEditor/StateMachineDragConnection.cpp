...
TSharedRef<FStateMachineDragConnection> FStateMachineDragConnection::New(const TSharedRef<SGraphPanel> &GraphPanel, const FDraggedPinTable &DraggedPins)
{
	TSharedRef<FStateMachineDragConnection> Operation = MakeShareable(new FStateMachineDragConnection(GraphPanel, DraggedPins));
	Operation->Construct();

	return Operation;
}

void FStateMachineDragConnection::OnDrop(bool bDropWasHandled, const FPointerEvent &MouseEvent)
{
...
void FStateMachineDragConnection::OnDragged(const class FDragDropEvent &DragDropEvent)
{
...
void FStateMachineDragConnection::HoverTargetChanged()
{
	...
	else if (UEdNode_StateMachineNode *TargetNodeObj = Cast<UEdNode_StateMachineNode>(GetHoveredNode()))
	{
		TArray<UEdGraphPin*> ValidSourcePins;
		ValidateGraphPinList(/*out*/ ValidSourcePins);
...
#define LOCTEXT_NAMESPACE "AssetSchema_StateMachine"
				Response = FPinConnectionResponse(CONNECT_RESPONSE_DISALLOW, LOCTEXT("PinError", "Not a valid UStateMachineEdNode"));
...
FStateMachineDragConnection::FStateMachineDragConnection(const TSharedRef<SGraphPanel> &GraphPanelIn, const FDraggedPinTable &DraggedPinsIn)
...
FReply FStateMachineDragConnection::DroppedOnPin(FVector2D ScreenPosition, FVector2D GraphPosition)
{
	TArray<UEdGraphPin*> ValidSourcePins;
	ValidateGraphPinList(/*out*/ ValidSourcePins);
...
FReply FStateMachineDragConnection::DroppedOnNode(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
FReply FStateMachineDragConnection::DroppedOnPanel(const TSharedRef< SWidget > &Panel, FVector2D ScreenPosition, FVector2D GraphPosition, UEdGraph &Graph)
{
	// Gather any source drag pins
...
void FStateMachineDragConnection::ValidateGraphPinList(TArray<UEdGraphPin*> &OutValidPins)
{
