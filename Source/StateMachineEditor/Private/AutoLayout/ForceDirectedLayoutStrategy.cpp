...
FBox2D UForceDirectedLayoutStrategy::LayoutOneTree(UStateMachineNode *RootNode, const FBox2D &PreTreeBound)
{
	...
	for (int32 IterrationNum = 0; IterrationNum < MaxIteration; ++IterrationNum)
	{
		...
		while (CurrLevelNodes.Num() != 0)
		{
			for (int32 i = 0; i < CurrLevelNodes.Num(); ++i)
			{
				...
				UEdNode_StateMachineNode *EdNode_ParentNode = EdGraph->NodeMap[Node];

				for (int32 j = 0; j < Node->ChildrenNodes.Num(); ++j)
				{
					NextLevelNodes.Add(Node->ChildrenNodes[j]);

					UEdNode_StateMachineNode *EdNode_ChildNode = EdGraph->NodeMap[Node->ChildrenNodes[j]];
